...
import { Github, Shield, Activity, RotateCw, Bot, ChevronRight, MessageSquare, Terminal, Zap } from 'lucide-react';
...
const FEATURES = [
  { icon: Bot, title: 'AI Crash Repair', desc: 'When your bot crashes, our AI automatically diagnoses the issue and attempts up to 3 repairs before alerting you.' },
  { icon: Github, title: 'GitHub Import', desc: 'Deploy directly from any public repository — we clone, install deps, and launch it automatically.' },
  ...
  { icon: Activity, title: 'Live Console', desc: 'Real-time output stream visible in your portal. See exactly what your bot is logging at all times.' },
    ...
    <div className="min-h-screen w-full bg-[#0b0b0f] text-[#d4d4e0] overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-30 flex items-center justify-between h-14 px-6 md:px-12 border-b border-white/[0.06] bg-[#0b0b0f]/90 backdrop-blur-md">
        <div className="flex items-center gap-2.5">
          <img src="/lumora-icon.png" alt="Lumora" className="h-6 w-6 object-contain" />
      ...
      <section className="max-w-6xl mx-auto px-6 md:px-12 pt-24 pb-20 flex flex-col lg:flex-row items-center gap-16">
        ...
        <div className="flex-1 lg:max-w-lg">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-white leading-[1.1] tracking-tight mb-5">
            ...
            <br />
            <span className="text-[#d4d4e0]/55">We handle the rest.</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-12">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-center gap-2 px-6 py-2.5 rounded-md text-sm font-semibold border border-white/[0.09] text-white/50 hover:text-white/80 hover:border-white/[0.14] transition-colors"
            >
        ...
        {/* Mock dashboard — bot card only, no fake console */}
          ...
          <div className="rounded-xl border border-white/[0.07] overflow-hidden bg-[#111116]">
            ...
            <div className="flex items-center gap-1.5 px-4 py-3 border-b border-white/[0.06] bg-[#0e0e13]">
              ...
              <div className="flex items-center gap-2 ml-3">
                <img src="/lumora-icon.png" alt="" className="h-3 w-3 object-contain opacity-40" />
                <span className="text-[10px] font-mono text-white/25">lumora.host — portal</span>
              </div>
            </div>

            <div className="p-5 space-y-3">
              {/* Bot card */}
              <div className="rounded-lg border border-white/[0.07] bg-[#0e0e13] p-4">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <div className="text-xs font-mono text-white/70 font-semibold">my-discord-bot.zip</div>
                    <div className="text-[10px] font-mono text-white/30 mt-0.5">Active deployment</div>
                  </div>
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-emerald-500/20 bg-emerald-500/[0.06]">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[['RESTARTS', '0'], ['UPTIME', '3d 4h'], ['CMD', 'node index.js']].map(([k, v]) => (
                    <div key={k} className="rounded-md bg-[#16161c] border border-white/[0.05] p-2">
                      <div className="text-[8px] font-mono text-white/25 mb-0.5">{k}</div>
                      <div className="text-[9px] font-mono text-white/55 truncate">{v}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* AI agent preview */}
              <div className="rounded-lg border border-white/[0.06] bg-[#0e0e13] p-3">
                <div className="flex items-center gap-2 mb-2.5">
                  <Zap className="h-3 w-3 text-[#5c6cf5]" />
                  <span className="text-[9px] font-mono text-white/30 tracking-wider">AI AGENT</span>
                </div>
                <div className="space-y-1.5">
                  <div className="rounded-md bg-[#16161c] px-2.5 py-1.5 text-[9px] font-mono text-white/40 max-w-[80%]">
                    Bot is online. All systems healthy.
                  </div>
                  <div className="rounded-md bg-[#3d4df0]/15 border border-[#5c6cf5]/15 px-2.5 py-1.5 text-[9px] font-mono text-[#9aa3f5] max-w-[80%] ml-auto text-right">
                    Why did it crash last time?
                  </div>
                  <div className="rounded-md bg-[#16161c] px-2.5 py-1.5 text-[9px] font-mono text-white/40 leading-relaxed">
                    Missing DISCORD_TOKEN — add it in Secrets tab and restart.
                  </div>
                </div>
              </div>
            </div>
      ...
      <section id="runtimes" className="border-y border-white/[0.06] bg-[#0e0e13]">
        <div className="max-w-6xl mx-auto px-6 md:px-12 py-12 flex flex-col md:flex-row items-center gap-6 md:gap-0">
          <p className="text-sm text-white/30 font-mono tracking-widest md:mr-12 shrink-0">SUPPORTED RUNTIMES</p>
          <div className="flex flex-col sm:flex-row gap-6 md:gap-14 w-full md:w-auto">
            {RUNTIMES.map(({ label, note }) => (
              <div key={label} className="flex items-center gap-3">
                <span className="h-1.5 w-1.5 rounded-full bg-[#5c6cf5]/50 shrink-0" />
                <div>
      ...
      <section id="features" className="max-w-6xl mx-auto px-6 md:px-12 py-24">
        <div className="mb-14">
          ...
          <p className="text-white/35 text-base">No infrastructure knowledge required. Deploy in under 60 seconds.</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-white/[0.04] rounded-xl overflow-hidden border border-white/[0.05]">
          {FEATURES.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="bg-[#0b0b0f] p-6 hover:bg-[#0e0e13] transition-colors">
              <Icon className="h-5 w-5 text-white/25 mb-4" />
      ...
      <section className="max-w-6xl mx-auto px-6 md:px-12 pb-24">
        <div className="rounded-xl border border-white/[0.07] bg-[#111116] px-10 py-12 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <p className="text-white/35 text-sm">Enter your staff-issued access key and have your bot live in minutes.</p>
          </div>
          <button
            onClick={() => setLocation('/login')}
      ...
      <footer className="border-t border-white/[0.06] px-6 md:px-12 py-7 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <img src="/lumora-icon.png" alt="" className="h-4 w-4 object-contain opacity-30" />
          <span className="font-mono text-xs text-white/25 tracking-widest">LUMORA</span>
        </div>
